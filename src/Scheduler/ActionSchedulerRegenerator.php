...
final class ActionSchedulerRegenerator implements RegeneratorInterface {
	...
	private function syncOrPurge( int $postId, \WP_Post $post ): void {
		...
		if ( 'publish' === $post->post_status && ! $this->visibility->hasPassword( $post ) ) {
			$this->enqueue( $postId );
	...
	private function enqueue( int $postId ): void {
		if ( as_has_scheduled_action( self::REGENERATE_HOOK, array( $postId ), self::GROUP ) ) {
			return;
		}

		as_enqueue_async_action( self::REGENERATE_HOOK, array( $postId ), self::GROUP );
	}
}
