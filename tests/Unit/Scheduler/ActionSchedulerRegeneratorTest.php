...
final class ActionSchedulerRegeneratorTest extends TestCase {
	...
	public function test_on_save_enqueues_when_publish_allowed_type_and_no_password(): void {
		...
		Functions\expect( 'as_enqueue_async_action' )
			->once()
			->with( ActionSchedulerRegenerator::REGENERATE_HOOK, array( 5 ), 'markout' );
	...
	public function test_on_delete_removes_cache(): void {
		...
		self::assertSame( array( 9 ), $cache->deletes );
	}
	...
	public function test_on_transition_deletes_cache_when_leaving_publish(): void {
		...
		self::assertSame( array( 11 ), $cache->deletes );
	}
	...
	public function test_on_transition_enqueues_when_becoming_publish(): void {
		...
		Functions\expect( 'as_enqueue_async_action' )
			->once()
			->with( ActionSchedulerRegenerator::REGENERATE_HOOK, array( 11 ), 'markout' );
	...
	public function test_handle_regenerate_syncs_valid_public_post(): void {
		...
		self::assertSame( array( 3 ), $cacher->syncedPostIds );
	}
	...
	public function test_handle_regenerate_deletes_cache_when_post_no_longer_exists(): void {
		...
		self::assertSame( array( 3 ), $cache->deletes );
	}
	...
	public function test_on_save_ignores_disallowed_post_types(): void {
		...
		self::assertSame( array(), $cache->deletes );
		self::assertSame( array(), $cacher->syncedPostIds );
	}
	...
	public function test_enqueue_dispatches_async_action_when_not_scheduled(): void {
		...
		Functions\when( 'as_enqueue_async_action' )->alias(
			static function ( string $hook, array $args, string $group ) use ( &$dispatched ): void {
				$dispatched = array( $hook, $args, $group );
			}
		);
		...
		self::assertSame(
			array( ActionSchedulerRegenerator::REGENERATE_HOOK, array( 42 ), 'markout' ),
			$dispatched
		);
	}
