...
final class FileCacheTest extends TestCase {
	...
	protected function tearDown(): void {
		if ( is_dir( $this->directory ) ) {
			...
			foreach ( $entries ? $entries : array() as $file ) {
				if ( is_dir( $file ) ) {
					$nestedEntries = glob( $file . '/*' );
					foreach ( $nestedEntries ? $nestedEntries : array() as $nested ) {
						@unlink( $nested );
					}
					@rmdir( $file );
					continue;
	...
	public function test_write_leaves_no_temp_files_behind(): void {
		...
		self::assertSame( array(), $leftovers );
	}
	...
	public function test_write_returns_false_and_cleans_up_when_rename_fails(): void {
		...
		try {
			...
			self::assertSame( array(), $leftovers, 'Temp file should be cleaned up after a failed rename.' );
		} finally {
